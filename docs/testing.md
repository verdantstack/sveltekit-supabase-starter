...
graph TB
    subgraph TEST["🧪 Test Run"]
        direction TB
        SUITE["vitest run"]
        FAKE["fake-supabase.ts<br/>in-memory JS maps<br/>fluent query surface"]
        SUITE -->|"each test"| FAKE
    end
    ...
    subgraph LAYERS["What Gets Tested"]
        ...
        AUDIT["Audit<br/>append-only writer"]
        SVC --> RBAC
        SVC --> BILL
        SVC --> AUDIT
    end
    ...
    subgraph RLS["RLS (separate, needs real DB)"]
        SUPA["supabase start<br/>real Postgres"]
    end

    TEST -.->|"not exercised<br/>by unit suite"| RLS

    style TEST fill:#e3f2fd,stroke:#1565c0
    style LAYERS fill:#e8f5e9,stroke:#2e7d32
    style RLS fill:#f3e5f5,stroke:#7b1fa2
```
...
- **Honest boundaries** — RLS policies are _not_ exercised by the unit suite; they need a real database.
