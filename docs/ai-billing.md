...
function calculateCost(record: UsageRecord): number {
  ...
  const rates = pricing[record.model] ?? { input: 1.0, output: 3.0 };
...
export async function chatCompletion(db: Db, orgId: string, messages: Anthropic.MessageParam[]) {
...
export async function checkUsageLimit(db: Db, orgId: string): Promise<UsageLimitResult> {
