...
A production-grade B2B SaaS foundation for SvelteKit + Supabase. Not a tutorial, not a boilerplate — a working implementation of multi-tenancy, RBAC, seat billing, audit logging, and Supabase RLS with 310 tests.
...
- RBAC / org / member / invite / billing / audit suites (310 tests) prove the business rules
...
- `npm run test:coverage` MUST pass before any commit — it enforces the 95% threshold
- Any new code MUST include tests to maintain the threshold
- **Excluded from coverage:** `supabase/client.ts` (infrastructure, not business logic)
